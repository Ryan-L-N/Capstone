...
# ─────────────────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="Testing Run 2 Policy in Baseline Environment")
...
parser.add_argument("--checkpoint", type=str, required=True, help="Path to trained navigation policy checkpoint")
...
class CircularWaypointEnv:
    ...
    def step(self, step_size: float) -> bool:
        ...
        # ═════════════════════════════════════════════════════════════════════
        # Uses Run 2 trained policy to generate navigation commands
        # ═════════════════════════════════════════════════════════════════════
        if self.current_waypoint_idx < len(self.waypoints):
            wp = self.waypoints[self.current_waypoint_idx]
            ...
            robot_vel = self.spot.robot.get_linear_velocity()
            robot_ang_vel = self.spot.robot.get_angular_velocity()
            
            yaw = quaternion_to_yaw(spot_orientation)

            # Build observation vector (32 dims) matching training format
            obs = []
            
            # Base velocity (3)
            obs.extend([robot_vel[0], robot_vel[1], robot_ang_vel[2]])
           
            # Heading (2)
            obs.extend([math.sin(yaw), math.cos(yaw)])
            
            # Waypoint info (3) - ROBOT FRAME (matches training)
            dx_world = target_x - spot_x
            dy_world = target_y - spot_y
            # Rotate world frame to robot frame
            ...
            obs_tensor = torch.from_numpy(np.array(obs, dtype=np.float32)).unsqueeze(0).to(device)
            
            with torch.no_grad():
                action, _, _ = navigation_policy.get_action(obs_tensor, deterministic=True)
            ...
            # Apply command to Spot
            command = np.array([action_scaled[0], action_scaled[1], action_scaled[2]])
...
def main():
    """Main policy testing loop."""
    print("\n" + "=" * 72)
    print("  TESTING RUN 2 POLICY IN BASELINE ENVIRONMENT")
    print("=" * 72)
