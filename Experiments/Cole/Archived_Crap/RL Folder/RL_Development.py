...
# ─────────────────────────────────────────────────────────────────────────────
# Lazy imports (only loaded when running as main script, not when imported)
# ─────────────────────────────────────────────────────────────────────────────
_ISAAC_IMPORTS_LOADED = False

def _ensure_isaac_imports():
    """Load Isaac Sim imports if not already loaded. Call this before using any Isaac classes."""
    global _ISAAC_IMPORTS_LOADED
    global omni, World, SpotFlatTerrainPolicy, Camera
    global UsdGeom, UsdLux, UsdPhysics, Gf, Sdf
    
    if not _ISAAC_IMPORTS_LOADED:
        import omni as _omni
        from omni.isaac.core import World as _World
        from omni.isaac.quadruped.robots import SpotFlatTerrainPolicy as _SpotPolicy
        from omni.isaac.sensor import Camera as _Camera
        from pxr import UsdGeom as _UsdGeom, UsdLux as _UsdLux, UsdPhysics as _UsdPhysics, Gf as _Gf, Sdf as _Sdf
        
        # Make them globally available
        globals()['omni'] = _omni
        globals()['World'] = _World
        globals()['SpotFlatTerrainPolicy'] = _SpotPolicy
        globals()['Camera'] = _Camera
        globals()['UsdGeom'] = _UsdGeom
        globals()['UsdLux'] = _UsdLux
        globals()['UsdPhysics'] = _UsdPhysics
        globals()['Gf'] = _Gf
        globals()['Sdf'] = _Sdf
        
        _ISAAC_IMPORTS_LOADED = True
...
COLOR_SMALL_OBSTACLE = (0.4, 0.4, 0.4)  # dark gray — visually distinct (tuple converted to Gf.Vec3f where needed)
...
# Colors (RGB tuples - converted to Gf.Vec3f where needed)
COLOR_LIGHT_OBSTACLE = (1.0,  0.55, 0.0)    # orange  - light/pushable obstacles
COLOR_HEAVY_OBSTACLE = (0.27, 0.51, 0.71)   # steel blue - heavy/immovable obstacles
# Medium obstacles use bronze/tan (0.8, 0.6, 0.2) - defined inline in spawn_one()
COLOR_WAYPOINT       = (1.0,  0.95, 0.0)    # bright yellow
COLOR_START_WAYPOINT = (0.2,  0.9,  0.2)    # bright green
COLOR_FLAG_POLE      = (0.88, 0.88, 0.88)   # light grey pole
...
def apply_rigid_body_physics(stage, prim_path: str, mass_kg: float,
    ...
    _ensure_isaac_imports()  # Load Isaac imports if not already loaded
...
class ObstacleManager:
    ...
    def spawn_one(self, idx: int, margin: float = 1.5, min_spawn_clearance: float = 2.0,
                  light_probability: float = 0.5, medium_probability: float = 0.3, 
                  heavy_probability: float = 0.2) -> None:
        """
        ...
        Args:
            idx: Obstacle index for naming
            margin: Arena margin for spawning
            min_spawn_clearance: Minimum distance between obstacles
            light_probability: Probability of spawning light obstacle (0-1)
            medium_probability: Probability of spawning medium obstacle (0-1)
            heavy_probability: Probability of spawning heavy obstacle (0-1)
        """
        ...
        # Sample mass from weight class based on probabilities
        # Normalize probabilities
        total_prob = light_probability + medium_probability + heavy_probability
        if total_prob <= 0:
            return  # No obstacles to spawn
        
        light_prob_norm = light_probability / total_prob
        medium_prob_norm = medium_probability / total_prob
        heavy_prob_norm = heavy_probability / total_prob
        
        # Choose weight class
        rand = self.rng.random()
        if rand < light_prob_norm:
            # Light obstacle
            mass = self.rng.uniform(OBSTACLE_MIN_MASS, OBSTACLE_LIGHT_MAX)
            weight_class = "light"
            color = COLOR_LIGHT_OBSTACLE
        elif rand < light_prob_norm + medium_prob_norm:
            # Medium obstacle
    ...
    def populate(self, target_coverage_pct: float = OBSTACLE_AREA_FRAC * 100,
        ...
        Args:
            target_coverage_pct: Target percentage of arena covered by obstacles
            min_spawn_clearance: Minimum distance between obstacles (meters)
            light_probability: Probability of spawning light obstacles (0-1)
            medium_probability: Probability of spawning medium obstacles (0-1)
            heavy_probability: Probability of spawning heavy obstacles (0-1)
        """
...
class CircularWaypointEnv:
    ...
    def __init__(self, world, stage, rng: np.random.Generator):
        ...
        # Curriculum learning: 4-stage training progression
        self.training_stage = 1  # Start at Stage 1 (no obstacles)
        # Stage 1: 0% obstacles
        # Stage 2: 10% light/medium moveable
        # Stage 3: 20% (10% light/medium + 10% heavy immovable)
        # Stage 4: 30% (10% light/medium + 10% heavy + 10% small static)
    ...
    def reset(self, episode: int) -> None:
        ...
        # Populate obstacles based on training stage (curriculum learning)
        self._spawn_obstacles_for_stage()
    ...
    def set_training_stage(self, stage: int) -> None:
        """Set the current training stage (1-4)."""
        if stage < 1 or stage > 4:
            raise ValueError(f"Training stage must be 1-4, got {stage}")
        self.training_stage = stage
        print(f"\n{'='*72}")
        print(f"[CURRICULUM] Advanced to Stage {stage}")
        print(f"{'='*72}")
    
    def _spawn_obstacles_for_stage(self) -> None:
        """Spawn obstacles according to current training stage.
        
        Stage 1: No obstacles (0% coverage)
        Stage 2: 10% light/medium moveable obstacles
        Stage 3: 10% light/medium + 10% heavy immovable (20% total)
        Stage 4: 10% light/medium + 10% heavy + 10% small static (30% total)
        """
        if self.training_stage == 1:
            print("[STAGE 1] No obstacles - waypoint navigation only")
            # No obstacles spawned
            
        elif self.training_stage == 2:
            print("[STAGE 2] Spawning 10% light/medium moveable obstacles")
            # 10% moveable obstacles (light + medium weight classes)
            self.obstacle_mgr.populate(
                target_coverage_pct=10.0,
                min_spawn_clearance=2.0,
                light_probability=0.5,    # 50% light
                medium_probability=0.5,   # 50% medium
                heavy_probability=0.0     # 0% heavy (exclude heavy obstacles)
            )
            
        elif self.training_stage == 3:
            print("[STAGE 3] Spawning 10% light/medium + 10% heavy obstacles (20% total)")
            # 10% moveable (light/medium)
            self.obstacle_mgr.populate(
                target_coverage_pct=10.0,
                min_spawn_clearance=2.0,
            ...
            self.obstacle_mgr.populate(
                ...
                heavy_probability=1.0
            )
            
        elif self.training_stage == 4:
            print("[STAGE 4] Full complexity: 10% light/medium + 10% heavy + 10% small static (30% total)")
            # 10% moveable (light/medium)
            self.obstacle_mgr.populate(
                target_coverage_pct=10.0,
                min_spawn_clearance=2.0,
...
def setup_spot_sensors(spot_prim_path: str):
    ...
    - 360 degree panoramic RGB vision
    - IMU (accessible via robot.get_linear_velocity(), get_angular_velocity())
    - Joint encoders (12 joints: 3/leg x 4 legs)
...
def main(simulation_app, args):
    """Main training loop."""
    _ensure_isaac_imports()  # Load Isaac Sim imports
