...
sys.path.insert(0, r"C:\Users\user\Desktop\Capstone_vs_1.2\Immersive-Modeling-and-Simulation-for-Autonomy\Experiments\Cole\RL_Folder_VS2")
...
if args.checkpoint:
    ...
    navigation_policy = NavigationPolicy(obs_dim=75, action_dim=3).to(device)
...
class CircularWaypointEnv:
    ...
    def step(self, step_size: float) -> bool:
        ...
        if navigation_policy is not None and self.current_waypoint_idx < len(self.waypoints):
            ...
            # Build observation vector (obs_dim=75) - VS3 format
            # Format (75 dims):
            # [0:3] Base velocity (vx, vy, omega)
            # [3:5] Heading (sin, cos)
            # [5:9] IMU (roll, pitch, accel_x, accel_y)
            # [9:12] Waypoint info (dx_robot, dy_robot, distance)
            # [12:60] Multi-layer obstacles (16 rays × 3 heights)
            # [60:64] Foot contact (4 feet)
            # [64:68] Leg joint angles (4 legs)
            # [68:75] Stage encoding (one-hot, 7 dims)
            ...
            try:
                ...
                obs[0] = lin_vel[0]  # vx
                obs[1] = lin_vel[1]  # vy
                obs[2] = ang_vel[2]  # omega (yaw rate)
            except:
                obs[0:3] = 0.0
            
            # [3:5] Heading (sin, cos of yaw)
            obs[3] = np.sin(current_heading)
            obs[4] = np.cos(current_heading)
            
            # [5:9] IMU data (roll, pitch, accel_x, accel_y)
            # Roll and pitch from orientation
            _, ori = self.spot.robot.get_world_pose()
            w, x, y, z = ori[0], ori[1], ori[2], ori[3]
            roll = np.arctan2(2*(w*x + y*z), 1 - 2*(x*x + y*y))
            pitch = np.arcsin(2*(w*y - z*x))
            obs[5] = roll
            obs[6] = pitch
            # Acceleration (simplified from velocity)
            if hasattr(self, '_last_vx'):
                obs[7] = lin_vel[0] - self._last_vx  # accel_x
                obs[8] = lin_vel[1] - self._last_vy  # accel_y
            else:
                obs[7:9] = 0.0
            self._last_vx = lin_vel[0]
            self._last_vy = lin_vel[1]
            ...
            cos_yaw = np.cos(current_heading)
            sin_yaw = np.sin(current_heading)
            obs[9] = dx_world * cos_yaw + dy_world * sin_yaw   # dx_robot
            obs[10] = -dx_world * sin_yaw + dy_world * cos_yaw  # dy_robot
            obs[11] = math.sqrt(dx_world**2 + dy_world**2)      # distance
            
            # [12:60] Multi-layer obstacle distances (16 rays × 3 heights = 48)
            # Raycast at 3 heights: low (-0.2), center (0.0), high (+0.2)
            obstacle_idx = 12
            for height_offset in [-0.2, 0.0, 0.2]:
                obstacle_dists = self._raycast_obstacles_3d(spot_x, spot_y, current_heading, height_offset)
                for dist in obstacle_dists:
                    obs[obstacle_idx] = min(dist, 10.0) / 10.0  # Normalize by ~max relevant distance
                    obstacle_idx += 1
            
            # [60:64] Foot contact (4 feet) - simplified, all 1.0 for standing
            obs[60:64] = 1.0
            
            # [64:68] Leg joint summary (4 legs) - simplified
            obs[64:68] = 0.0  # or could track from kinematics
            
            # [68:75] Stage encoding (one-hot) - set to stage 1 (index 0)
            obs[68] = 1.0  # Stage 1 is index 0 in one-hot encoding
            ...
            # Action format: [forward_vel, strafe_vel, turn_rate]
            # Policy outputs should be roughly in [-1, 1] range
            vx = np.clip(action[0], -1.0, 1.0) * SPOT_MAX_SPEED_MPS
            vy = np.clip(action[1], -1.0, 1.0) * SPOT_MAX_SPEED_MPS * 0.5  # Reduce max strafe
            omega = np.clip(action[2], -1.0, 1.0) * MAX_TURN_RATE
            
            command = np.array([vx, vy, omega])
            self.spot.forward(step_size, command)

        return True

    def _raycast_obstacles_3d(self, robot_x: float, robot_y: float, 
                               robot_yaw: float, height_offset: float = 0.0) -> list:
        """
        Raycast in 16 directions to detect obstacles at specified height.
        VS3 observation format: 16 rays × 3 heights.
        
        Args:
            robot_x, robot_y: robot position in world frame
            robot_yaw: robot heading in world frame
            height_offset: height above robot center for this raycast layer
            
        Returns:
            list of 16 normalized obstacle distances [0, 1]
        """
        num_rays = 16
        ray_length = 10.0  # meters, should match nav_config
        ray_angles = np.linspace(0, 2 * np.pi, num_rays, endpoint=False)
        
        obstacle_dists = []
        robot_z = 0.5 + height_offset  # Spot center height ~ 0.5m
        
        for ray_angle in ray_angles:
            # Ray direction in world frame
            global_dir_x = np.cos(robot_yaw + ray_angle)
            global_dir_y = np.sin(robot_yaw + ray_angle)
            
            # Ray endpoint
            ray_end_x = robot_x + ray_length * global_dir_x
            ray_end_y = robot_y + ray_length * global_dir_y
            
            # Check distance to nearest obstacle from obstacle manager
            # Simple approach: use existing obstacle manager if available
            if hasattr(self, 'obstacle_mgr') and self.obstacle_mgr:
                # Distance to nearest obstacle in XY plane
                min_dist = ray_length
                for obs in self.obstacle_mgr.obstacles:
                    try:
                        obs_x, obs_y = obs['pos'][0], obs['pos'][1]
                        # Get obstacle half-width for collision check
                        obs_w = obs['dims'][0] / 2 if 'dims' in obs else 0.2
                        obs_d = obs['dims'][1] / 2 if 'dims' in obs else 0.2
                        obs_h = obs['dims'][2] if 'dims' in obs else 0.5
                        
                        # Height check (raycast is at robot_z, check if obstacle overlaps this height)
                        obs_bottom = 0.0  # obstacles spawn on ground
                        obs_top = obs_h
                        if robot_z >= obs_bottom and robot_z <= obs_top:  # Height overlap
                            # Distance to nearest edge of obstacle in ray direction
                            obs_to_robot_x = robot_x - obs_x
                            obs_to_robot_y = robot_y - obs_y
                            max_obstacle_extent = max(obs_w, obs_d)
                            dist_to_obs = math.sqrt(obs_to_robot_x**2 + obs_to_robot_y**2) - max_obstacle_extent
                            if dist_to_obs < min_dist and dist_to_obs > 0.01:
                                min_dist = dist_to_obs
                    except (KeyError, IndexError, TypeError):
                        continue
                obstacle_dists.append(min(min_dist, ray_length))
            else:
                obstacle_dists.append(ray_length)  # No obstacles detected
