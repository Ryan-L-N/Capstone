...
if args.checkpoint:
    ...
    navigation_policy = NavigationPolicy(obs_dim=32, action_dim=3).to(device)
...
def raycast_obstacle_dists(spot_x: float, spot_y: float, yaw: float,
                           obstacle_mgr: 'ObstacleManager', num_rays: int = 16,
                           ray_length: float = 5.0) -> np.ndarray:
    """
    Perform 16-direction raycasting to detect obstacles (matching training env).
    
    Args:
        spot_x, spot_y: robot position
        yaw: robot heading (radians)
        obstacle_mgr: obstacle manager for collision checks
        num_rays: number of rays (default 16)
    ...
    for i in range(num_rays):
        ...
        # Check distance to obstacles (simplified - check nearest obstacle in approximate direction)
        if obstacle_mgr.obstacles:
            nearest_obs_dist = 999.0
            for obs in obstacle_mgr.obstacles:
                obs_dist = distance_2d([spot_x, spot_y], obs["pos"])
                if obs_dist < nearest_obs_dist:
                    nearest_obs_dist = obs_dist
            # Use minimum of boundary and obstacle distance (with some margin)
            dist = min(dist, max(0.1, nearest_obs_dist - 0.5))
...
class CircularWaypointEnv:
    ...
    def step(self, step_size: float) -> bool:
        ...
        if navigation_policy is not None and self.current_waypoint_idx < len(self.waypoints):
            ...
            # Build observation vector EXACTLY matching training format (obs_dim=32)
            # Training format:
            #   - Base velocity (3): vx, vy, omega
            #   - Heading (2): sin(yaw), cos(yaw)
            #   - Waypoint in ROBOT frame (3): dx_robot, dy_robot, distance
            #   - Obstacle raycasts (16): 16-ray obstacle detection
            ...
            # (3) Waypoint info in ROBOT FRAME (critical fix!)
            # First compute world-frame offset
            dx_world = target_x - spot_x
            dy_world = target_y - spot_y
            distance = np.sqrt(dx_world**2 + dy_world**2)
            
            # Rotate to robot frame using coordinate transformation
            ...
            # (16) Obstacle distances from 16-ray raycasting
            obstacle_distances = raycast_obstacle_dists(spot_x, spot_y, current_heading,
                                                        self.obstacle_mgr, num_rays=16, ray_length=5.0)
            obs[obs_idx:obs_idx + 16] = obstacle_distances
            obs_idx += 16
            
            # (8) Stage encoding - one-hot vector
            stage_one_hot = np.zeros(8, dtype=np.float32)
            stage_one_hot[min(training_stage, 7)] = 1.0  # Use training stage, clamp to [0, 7]
            obs[obs_idx:obs_idx + 8] = stage_one_hot
            obs_idx += 8
            
            # Verify observation size
            assert obs_idx == 32, f"Observation size mismatch: {obs_idx} != 32"
