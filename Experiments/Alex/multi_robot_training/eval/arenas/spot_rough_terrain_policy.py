...
Observation:  235-dim = 48 proprioception + 187 height scan.

The 187 height-scan dims use PhysX raycasting to detect terrain ahead of the
robot, matching the Isaac Lab training pipeline. A 17x11 grid (1.6m x 1.0m,
0.1m resolution) is cast downward from the body frame, rotated by yaw only.
Formula: height = body_z - hit_z - 0.5, clipped to [-1, 1].
...
# PhysX scene query for raycasting
...
DEFAULT_CHECKPOINT = (
    r"C:\Users\Gabriel Santiago\OneDrive\Desktop\Nvidia Omniverse"
    r"\AI2C_Tech_Capstone_MS_for_Autonomy\MS_for_autonomy"
    r"\experimental_design_grass\code\checkpoints\model_3700.pt"
)
...
# Pre-compute body-frame grid offsets (xy ordering to match Isaac Lab)
# X: -0.8 to +0.8, Y: -0.5 to +0.5, step 0.1
_grid_x = np.arange(-SCAN_SIZE_X / 2, SCAN_SIZE_X / 2 + 1e-9, SCAN_RES)
_grid_y = np.arange(-SCAN_SIZE_Y / 2, SCAN_SIZE_Y / 2 + 1e-9, SCAN_RES)
_gx, _gy = np.meshgrid(_grid_x, _grid_y, indexing='xy')
# Shape (187, 2) — each row is (dx, dy) offset from body origin in body frame
SCAN_GRID_BODY = np.stack([_gx.ravel(), _gy.ravel()], axis=-1).astype(np.float32)
...
class SpotRoughTerrainPolicy:
    ...
    # ------------------------------------------------------------------
    # Height scanner (PhysX raycasting)
    # ------------------------------------------------------------------
    def _cast_height_rays(self):
        ...
        3. Place ray origins at body_xy + rotated_offset, z = body_z + 20m
        4. Cast rays downward (0, 0, -1) using PhysX scene queries
        5. Compute: height = body_z - hit_z - 0.5
        6. Clip to [-1.0, 1.0]

        Returns:
            np.ndarray (187,): height scan values in meters, clipped to [-1, 1].
        """
        pos, quat = self.robot.get_world_pose()
        body_z = float(pos[2])

        # Extract yaw from quaternion (ignore roll/pitch, matches training)
        w, x, y, z = float(quat[0]), float(quat[1]), float(quat[2]), float(quat[3])
        yaw = np.arctan2(2.0 * (w * z + x * y), 1.0 - 2.0 * (y * y + z * z))
        cos_yaw = np.cos(yaw)
        sin_yaw = np.sin(yaw)

        # Rotate body-frame grid offsets by yaw only
        # world_dx = cos*dx - sin*dy, world_dy = sin*dx + cos*dy
        dx = SCAN_GRID_BODY[:, 0]
        dy = SCAN_GRID_BODY[:, 1]
        world_dx = cos_yaw * dx - sin_yaw * dy
        world_dy = sin_yaw * dx + cos_yaw * dy

        # Ray origins: body position + rotated offset + 20m height
        ray_origin_x = float(pos[0]) + world_dx
        ray_origin_y = float(pos[1]) + world_dy
        ray_origin_z = body_z + SCAN_RAY_HEIGHT
            ...
            if hit["hit"]:
                ...
                if "/World/Spot" in hit_path:
                    ...
                    if hit2["hit"]:
                        ...
                        if "/World/Spot" not in hit2_path:
                            heights[i] = body_z - hit2["position"][2] - SCAN_OFFSET
                else:
                    heights[i] = body_z - hit["position"][2] - SCAN_OFFSET
            # else: stays at SCAN_FILL_VAL (0.0 = assume flat)

        # Clip to match training observation range
    ...
    def initialize(self):
        ...
        # Height scan: PhysX raycasting (17x11 grid, 1.6m x 1.0m)
        print(f"[ROUGH] Height scan: PhysX raycast "
              f"({SCAN_NX}x{SCAN_NY}={SCAN_N} rays, "
              f"grid {SCAN_SIZE_X}x{SCAN_SIZE_Y}m, res {SCAN_RES}m)")
