...
class SpotRewardsCfg:
    ...
    # -- Task rewards (positive) -- Paper-matched coefficients --
    ...
    base_angular_velocity = RewardTermCfg(
        ...
        weight=6.0,  # was 5.0 — AI coach iter 3300: boost directional control after human visual eval
    ...
    gait = RewardTermCfg(
        ...
        weight=8.5,  # was 10.0 — AI coach iter 1200: reduce gait dominance to unlock terrain
        params={
    ...
    action_smoothness = RewardTermCfg(func=clamped_action_smoothness_penalty, weight=-1.2)  # was -1.0 — AI coach iter 2500: smoother gait per human eval
    ...
    base_motion = RewardTermCfg(
        ...
        weight=-2.4,  # was -2.0 — AI coach iter 2500: reduce instability per human eval
        params={"asset_cfg": SceneEntityCfg("robot")},
    )
    base_orientation = RewardTermCfg(
        func=spot_mdp.base_orientation_penalty,
        weight=-2.4,  # was -3.0 — AI coach iter 1200: reduce competing penalty during terrain unlock
        params={"asset_cfg": SceneEntityCfg("robot")},
    )
...
@configclass
class SpotLocomotionEnvCfg(LocomotionVelocityRoughEnvCfg):
...
@configclass
class SpotTeacherObservationsCfg(SpotObservationsCfg):
    """Teacher observations with privileged terrain information."""

    @configclass
    class PolicyCfg(SpotObservationsCfg.PolicyCfg):
...
@configclass
class SpotTeacherEnvCfg(SpotLocomotionEnvCfg):
