...
class MetricsSnapshot:
    ...
    # Velocity tracking (gait quality proxy)
    vel_tracking_error_xy: float = 0.0
    vel_tracking_error_yaw: float = 0.0
...
class MetricsCollector:
    ...
    def collect(self, iteration: int, elapsed_hours: float,
        ...
        # Velocity tracking errors (gait quality proxy)
        vel_error_xy = reward_info.get("Metrics/base_velocity/error_vel_xy", 0.0)
        vel_error_yaw = reward_info.get("Metrics/base_velocity/error_vel_yaw", 0.0)
        ...
        snapshot = MetricsSnapshot(
            ...
            vel_tracking_error_xy=vel_error_xy,
            vel_tracking_error_yaw=vel_error_yaw,
        )
    ...
    def is_plateau(self, metric: str = "mean_terrain_level",
                   min_snapshots: int = 3, threshold: float = 0.05) -> bool:
        """Detect if a metric has stalled (near-zero slope).

        Args:
            metric: Attribute name on MetricsSnapshot.
            min_snapshots: Minimum number of snapshots needed (each snapshot
                is one coach consultation, typically 100 iters apart).
            threshold: Max absolute slope to be considered stalled.
        """
        if len(self.history) < min_snapshots:
            return False
        values = [getattr(s, metric) for s in list(self.history)[-min_snapshots:]]
        # Check range — if terrain moved less than 0.15 over the window, it's stuck
        val_range = max(values) - min(values)
        return val_range < 0.15
