...
**v8 fix:** The coach now sees `vel_tracking_error_xy` and `vel_tracking_error_yaw` in every consultation. These come from `Metrics/base_velocity/error_vel_xy` in the env extras — data that was always there but never surfaced to the coach. The troubleshooting guide now includes:

| Symptom | Cause | Fix |
|---------|-------|-----|
| `vel_tracking_error_xy > 3.0` | Penalties overpower velocity rewards | Increase velocity rewards or reduce penalties |
| Velocity error spikes after weight change | Recent changes broke directional control | Roll back or boost velocity rewards |

This means the coach can now detect "robot can't walk forward" as a number — no human lava-arena session needed.
