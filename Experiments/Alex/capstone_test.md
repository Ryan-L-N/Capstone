...
- **Manual Teleop:** 4 environments x 1 robot with Xbox controller = **4 walkthrough sessions** (real-time, gait switching, FPV camera)
- **Total automated:** 8,080 episodes, estimated wall-clock time: **~5-5.5 hours** + manual walkthrough time
...
### 6.7 Manual Xbox Controller Walkthrough

Run each environment as a single-lane, single-robot rendered session controlled via Xbox controller. This provides hands-on qualitative assessment and allows the operator to feel the terrain challenges firsthand while hot-swapping between flat and rough gaits in real time.

**Purpose:** Supplement the automated statistical runs with human-in-the-loop exploration. Capture video of an operator manually navigating each environment, switching gaits at will, and using FPV to observe foot placement and obstacle interaction up close.

#### Xbox Controller Mapping

Reuses the existing input configuration from `spot_obstacle_course.py` (lines 172-181) and `spot_teleop.py` (lines 168-178):

| Input | Action | Constant |
|-------|--------|----------|
| **Left Stick Y** | Forward / backward (analog, inverted) | `XBOX_AXIS_FWD = 1` |
| **Left Stick X** | Turn left / right (analog) | `XBOX_AXIS_TURN = 0` |
| **A Button** | Cycle drive mode (MANUAL → SMOOTH → PATROL → AUTO-NAV) | `XBOX_BTN_A = 0` |
| **B Button** | Toggle selfright (rollover recovery) | `XBOX_BTN_B = 1` |
| **Y Button** | Reset robot position to zone start | `XBOX_BTN_Y = 3` |
| **LB Button** | Toggle FPV camera (first-person onboard) | `XBOX_BTN_LB = 4` |
| **RB Button** | **Cycle gait mode (FLAT ↔ ROUGH)** | `XBOX_BTN_RB = 5` |
| **D-Pad Up/Down** | Adjust speed multiplier (±0.002/frame) | — |
| **Back Button** | Emergency stop | `XBOX_BTN_BACK = 6` |
| Deadzone | 12% on both analog sticks | `XBOX_DEADZONE = 0.12` |

**Keyboard fallback** (from `spot_teleop.py`):
...
Hot-swap between the two policies at any point during traversal using RB (controller) or G (keyboard). Reuses the gait switching architecture from `spot_obstacle_course.py` (lines 1071-1105, 1439-1451):

```python
# Gait mode definitions (from spot_obstacle_course.py:231-243)
GAIT_MODES = [
    {"name": "FLAT",  "policy_type": "flat",
     "description": "Flat terrain policy (fast, efficient)"},
    {"name": "ROUGH", "policy_type": "rough",
     "description": "Rough terrain policy (stairs, rubble, obstacles)"},
]

# On gait switch:
# 1. Save current policy's PhysX properties (gains, friction, efforts)
# 2. Apply incoming policy's gains via apply_gains()
# 3. Zero commands for GAIT_SWITCH_STABILIZE frames (~0.5s) to let policy settle
# 4. Resume operator control
```

The stabilization pause (from `spot_obstacle_course.py:1546-1550`) prevents the robot from jerking during policy transitions:

```python
GAIT_SWITCH_STABILIZE = 0.5  # seconds of zero-command after switch
...
Four drive modes available via A button, reusing `spot_teleop.py` (lines 180-226):

| Mode | Max Fwd | Max Turn | Smoothing | Use Case |
|------|---------|----------|-----------|----------|
| MANUAL | 1.5 m/s | 1.0 rad/s | Off | Direct response, aggressive testing |
| SMOOTH | 1.2 m/s | 0.8 rad/s | On (accel=1.5, decel=3.0) | Realistic robot inertia |
| PATROL | 0.6 m/s | 0.5 rad/s | On (accel=1.0, decel=2.5) | Slow, careful navigation for boulders/stairs |
| AUTO-NAV | 1.0 m/s | 0.8 rad/s | On (accel=1.2, decel=3.0) | Autonomous + WASD override |

#### FPV Camera

Toggle between orbit (default) and first-person onboard view via LB (controller) or M (keyboard). Reuses the FPV camera from `spot_teleop.py` (lines 897-906) and `spot_obstacle_course.py` (lines 1384-1391):

```python
# FPV camera: 18mm wide-angle lens mounted on Spot's body
# Position: 0.4m forward, 0.15m up from body center (front of head)
fpv_cam_path = "/World/Spot/body/fpv_camera"
fpv_cam = UsdGeom.Camera.Define(stage, fpv_cam_path)
fpv_cam.CreateFocalLengthAttr(18.0)          # Wide FOV for immersive view
...
# Rotate: USD cameras look -Z, rotate so it looks +X (forward) with +Z up
cam_xform.AddOrientOp().Set(Gf.Quatf(0.5, 0.5, -0.5, -0.5))

# Toggle handler (LB button or M key)
def toggle_fpv():
...
#### Arena Configuration (Single Lane)

Each environment is set up as a single 30m x 50m arena with one robot:

| Parameter | Value |
|-----------|-------|
| Arena dimensions | 30m x 50m (same as statistical runs) |
| Number of robots | 1 |
| Spawn position | x=0, y=15m (center) |
| Physics | 500 Hz, GPU PhysX (`device="cuda:0"`) |
| Rendering | 50 Hz (real-time) |
| Policy loading | Both flat and rough loaded, hot-swappable |
| Controller | Xbox (pygame XInput) with keyboard fallback |
| Video capture | Optional screen recording |

#### Execution

```bash
# Launch manual teleop walkthrough for each environment
# Requires display + Xbox controller connected

# Friction surface walkthrough
./isaaclab.sh -p run_capstone_teleop.py --env friction --device cuda:0

# Grass / fluid resistance walkthrough
./isaaclab.sh -p run_capstone_teleop.py --env grass --device cuda:0

# Boulder field walkthrough
./isaaclab.sh -p run_capstone_teleop.py --env boulder --device cuda:0

# Staircase walkthrough
./isaaclab.sh -p run_capstone_teleop.py --env stairs --device cuda:0
```

#### Suggested Walkthrough Protocol

For each environment, perform the following sequence:

1. **Flat gait full run:** Start in FLAT gait, SMOOTH drive mode. Attempt full 50m traversal. Note where the policy struggles.
2. **Rough gait full run:** Reset (Y button), switch to ROUGH gait (RB). Attempt full 50m traversal. Compare behavior.
3. **FPV inspection:** Reset, enable FPV (LB). Walk through zones 3-5 in PATROL mode to inspect foot placement and obstacle interaction from the robot's perspective.
4. **Gait comparison at failure point:** Navigate to the zone where the flat policy failed. Switch between FLAT and ROUGH (RB) repeatedly to observe the behavioral difference at that specific terrain challenge.
5. **Screen record** each walkthrough for documentation and presentation.

#### Reference Files

| File | Path | What to Reuse |
|------|------|---------------|
| Xbox mapping + FPV + gait switch | `ARL_DELIVERY/02_Obstacle_Course/spot_obstacle_course.py` | Full controller loop, gait hot-swap, FPV camera |
| Xbox mapping + drive modes + FPV | `ARL_DELIVERY/04_Teleop_System/spot_teleop.py` | Drive modes, pygame setup, analog deadzone |
| Rough policy deployment wrapper | `ARL_DELIVERY/03_Rough_Terrain_Policy/spot_rough_terrain_policy.py` | `apply_gains()`, `forward()`, observation computation |
| Keyboard teleop fallback | `ARL_DELIVERY/02_Obstacle_Course/play_rough_teleop.py` | WASD velocity commands, smooth acceleration |
...
├── run_capstone_eval.py          # Main entry point (headless + rendered auto)
├── run_capstone_teleop.py        # Manual Xbox controller walkthrough
├── envs/
│   ├── __init__.py
